...
class trayIcon:
  ...
  def __init__(self):
    #Get the max freq
    self.maxFreq = updateThread.getMaxFreq();
...
class updateThead ( threading.Thread ):
  def run ( self ):
    return
  ...
  def getMaxFreq(self):
    ...
    # Max freq value
    p = subprocess.Popen('cpufreq-info -l | awk \'{print $2}\'', stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
